    ...
    Effect.gen(function* () {
      ...
      yield* runCommand(
        ChildProcess.make(spawnCommand.command, spawnCommand.args, {
          ...
          env: {
            ...process.env,
            COLON3CODE_PACK_EXE: "1",
            ...Option.match(config.target, {
              onNone: () => ({}),
              onSome: (target) => ({ COLON3CODE_PACK_EXE_TARGET: target }),
            }),
          },
          stdout: config.verbose ? "inherit" : "ignore",
          stderr: "inherit",
 ...
 * every `@colon3code/t3-<platform>.tgz` first, `t3.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@colon3code");
  ...
  Command.withDescription(
    "Publish the @colon3code/t3-<platform> tarballs and then the t3 launcher to npm.",
  ),
);
