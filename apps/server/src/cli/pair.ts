 ...
 * own `.colon3code` is checked first (matching dev-runner precedence); otherwise the
...
export class NoRunningServerError extends Schema.TaggedError<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running :3 Code server found.",
...
export class ServesOtherEnvironmentError extends Schema.TaggedError<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different :3 Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedError<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a :3 Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // Same precedence as dev-runner: inside a linked worktree its own `.colon3code`
    ...
    const envHome = yield* Config.string("COLON3CODE_HOME").pipe(Config.option);
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running :3 Code server and print it as a QR code.",
  ),
