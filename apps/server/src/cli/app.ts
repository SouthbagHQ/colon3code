...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the :3 Code desktop app. Start or update the desktop app on this machine, then run `t3 app` again. A running :3 Code server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedError<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `:3 Code could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  yield* Console.log(`Opened ${workspaceRoot} in :3 Code.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running :3 Code desktop app."),
