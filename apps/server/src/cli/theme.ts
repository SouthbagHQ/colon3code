...
export class ThemeFileInvalidError extends Schema.TaggedError<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from :3 Code, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
const envT3Home = Config.string("COLON3CODE_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then COLON3CODE_HOME,
  // then the default home. A provisioning script exporting COLON3CODE_HOME must
