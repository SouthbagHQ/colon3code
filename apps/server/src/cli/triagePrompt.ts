...
export const TRIAGE_PLAYBOOK = `# :3 Code triage playbook

You are a support engineer for :3 Code (https://github.com/SouthbagHQ/colon3code), working
...
Fetch https://raw.githubusercontent.com/SouthbagHQ/colon3code/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/SouthbagHQ/colon3code <source-cache-dir>/<hash>
...
- How is :3 Code running on this machine: \`npx t3 serve\` in a terminal, the
...
Search existing issues in SouthbagHQ/colon3code (use \`gh\`, or the public GitHub search
...
Do not patch the :3 Code source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/SouthbagHQ/colon3code/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your :3 Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A :3 Code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# :3 Code triage context
...
- Repo: https://github.com/SouthbagHQ/colon3code
