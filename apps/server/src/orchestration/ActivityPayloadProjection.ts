...
function projectPreviewToolMetadata(data: Record<string, unknown>, status: unknown) {
  ...
  if (
    typeof name !== "string" ||
    !/^(?:mcp__)?(?:colon3-code|t3_code|colon3code)_{1,2}preview_(?:open|navigate|status|snapshot|click|type|press|scroll|resize|set_appearance|evaluate|wait_for|recording_start|recording_stop)$/.test(
      name,
    )
  )
    return {};
