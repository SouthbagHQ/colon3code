...
describe("CheckpointReactor", () => {
  ...
  it("re-asks for the pull request at turn end when the thread branch is checked out", async () => {
    ...
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "colon3code/feature",
      localStatusRefName: "colon3code/feature",
  ...
  it("re-asks for the pull request after adopting a drifted checkout", async () => {
    ...
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "colon3code/original-branch",
      localStatusRefName: "colon3code/renamed-by-agent",
      pullRequestRefreshCalls,
    });
    ...
    await waitForEvent(
      ...
      (event) =>
        ...
        (event as unknown as { payload: { branch?: string } }).payload.branch ===
          "colon3code/renamed-by-agent",
    );

    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("colon3code/renamed-by-agent");
  });
  ...
  it("follows a checkout from a saved placeholder branch and refreshes its pull request", async () => {
    ...
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "colon3code/fd9cbe0e",
  ...
  it.each(["colon3code/original-branch", "colon3code/fd9cbe0e"])(
  ...
  it("does not adopt a temporary placeholder checkout as the thread branch", async () => {
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "colon3code/original-branch",
      localStatusRefName: "colon3code/0a1b2c3d",
    });
    ...
    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("colon3code/original-branch");
  });
