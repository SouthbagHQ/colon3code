    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "colon3code-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const olderWorkspace = yield* makeTempDir("colon3code-workspace-older-");
        const newerWorkspace = yield* makeTempDir("colon3code-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const workspace = yield* makeTempDir("colon3code-workspace-");
        const otherWorkspace = yield* makeTempDir("colon3code-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const workspace = yield* makeTempDir("colon3code-workspace-");
        const linkParent = yield* makeTempDir("colon3code-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const backingUpper = yield* makeTempDir("colon3code-backing-upper-");
        const backingLower = yield* makeTempDir("colon3code-backing-lower-");
        const aliasParent = yield* makeTempDir("colon3code-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("colon3code-claude-legacy-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("colon3code-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("colon3code-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("colon3code-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("colon3code-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("colon3code-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const otherCodexHome = yield* makeTempDir("colon3code-codex-other-");
        const workspace = yield* makeTempDir("colon3code-workspace-");
        const otherWorkspace = yield* makeTempDir("colon3code-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-legacy-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("colon3code-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("colon3code-codex-env-");
        const claudeWorkspace = yield* makeTempDir("colon3code-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("colon3code-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("colon3code-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("colon3code-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("colon3code-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("colon3code-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and T3 data directory", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const configBaseDir = yield* makeTempDir("colon3code-scanner-base-");
        const workspace = yield* makeTempDir("colon3code-workspace-");
    ...
    it.effect("excludes Codex scratch directories and Downloads", () =>
      Effect.gen(function* () {
        ...
        const runId = path.basename(yield* makeTempDir("colon3code-scanner-test-"));
        ...
        const keep = yield* makeTempDir("colon3code-workspace-keep-");
    ...
    it.effect("skips linked git worktrees and reports the origin of real checkouts", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const repo = yield* makeTempDir("colon3code-workspace-repo-");
        const worktree = yield* makeTempDir("colon3code-workspace-worktree-");
        const plain = yield* makeTempDir("colon3code-workspace-plain-");
        const noRemote = yield* makeTempDir("colon3code-workspace-noremote-");
        const submodule = yield* makeTempDir("colon3code-workspace-submodule-");

        yield* fileSystem.makeDirectory(path.join(repo, ".git"));
        yield* fileSystem.writeFileString(
          path.join(repo, ".git", "config"),
          '[core]\n\tbare = false\n[remote "origin"]\n\turl = git@github.com:southbaghq/colon3code.git\n\tfetch = +refs/heads/*:refs/remotes/origin/*\n',
        );
        yield* fileSystem.writeFileString(
        ...
        expect(
          ...
          { path: plain, git: null },
          {
            path: repo,
            git: {
              remoteKey: "github.com/southbaghq/colon3code",
    ...
    it.effect(
      "excludes sandboxes under the configured worktrees dir without .colon3code in the path",
      () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
          ...
          // dir contains no `.colon3code` segment — only the config-based prefix match
          // can exclude this one.
          const worktreeCwd = path.join(configBaseDir, "worktrees", "colon3code", "wt-2");
          yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
          yield* writeTranscript({
            filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
            contents: claudeSessionLine(worktreeCwd),
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "colon3code", "wt-3");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("colon3code-metadata-home-");
        const secondHome = yield* makeTempDir("colon3code-metadata-second-");
        const codexHomePath = yield* makeTempDir("colon3code-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("colon3code-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("colon3code-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("colon3code-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("colon3code-short-metadata-codex-");
        const workspace = yield* makeTempDir("colon3code-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("colon3code-record-metadata-home-");
        const secondHome = yield* makeTempDir("colon3code-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("colon3code-record-metadata-codex-");
        const workspace = yield* makeTempDir("colon3code-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("colon3code-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("colon3code-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("colon3code-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("colon3code-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("colon3code-record-limit-codex-");
          const workspace = yield* makeTempDir("colon3code-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const customHome = yield* makeTempDir("colon3code-codex-custom-");
        const workspace = yield* makeTempDir("colon3code-workspace-");
    ...
    it.effect("streams large transcripts across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("colon3code-budget-claude-");
        const codexHomePath = yield* makeTempDir("colon3code-budget-codex-");
        const workspace = yield* makeTempDir("colon3code-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("colon3code-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("colon3code-record-budget-codex-");
        const workspace = yield* makeTempDir("colon3code-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("colon3code-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("colon3code-completed-claude-");
        const codexHomePath = yield* makeTempDir("colon3code-completed-codex-");
        const workspace = yield* makeTempDir("colon3code-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const workspace = yield* makeTempDir("colon3code-workspace-");
        const nonFilePath = yield* makeTempDir("colon3code-non-file-");
    ...
    it.effect("does not import sessions from a T3-managed worktree", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const configBaseDir = yield* makeTempDir("colon3code-scanner-base-");
        const workspace = path.join(configBaseDir, "worktrees", "colon3code", "managed-worktree");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const sharedHome = yield* makeTempDir("colon3code-codex-shared-");
        const workspace = yield* makeTempDir("colon3code-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("colon3code-claude-home-");
        const codexHomePath = yield* makeTempDir("colon3code-codex-home-");
        const oldWorkspace = yield* makeTempDir("colon3code-workspace-old-");
        const recentWorkspace = yield* makeTempDir("colon3code-workspace-recent-");
        const recentHome = yield* makeTempDir("colon3code-claude-recent-home-");
