...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("refreshes the Git root only when requested", () => {
    ...
    let rootPath = "/repo";
    let remoteUrl = "git@github.com:SouthbagHQ/colon3code.git";
    let refinements = 0;
    ...
    return Effect.gen(function* () {
      ...
      expect(first?.canonicalKey).toBe("github.com/southbaghq/colon3code");
          ...
          return {
            stdout: rootLookup
              ? failed
                ? ""
                : "/repo\n"
              : "origin\tgit@github.com:SouthbagHQ/colon3code.git (fetch)\n",
  ...
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:SouthbagHQ/colon3code.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/southbaghq/colon3code");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("southbaghq/colon3code");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("southbaghq");
      expect(identity?.name).toBe("colon3code");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:SouthbagHQ/colon3code.git"]);
  ...
  it.effect.each(["add", "replace"] as const)(
    ...
    (change) =>
      Effect.gen(function* () {
        ...
        yield* git(cwd, ["init"]);
        yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/colon3code.git"]);
        if (change === "replace") {
        ...
        expect(initialIdentity?.canonicalKey).toBe(
          change === "add" ? "github.com/julius/colon3code" : "github.com/t3tools/previous",
        );
        ...
        yield* git(cwd, [
          ...
          "upstream",
          "git@github.com:SouthbagHQ/colon3code.git",
        ]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, [
        ...
        "add",
        "origin",
        "git@gitlab.com:T3Tools/platform/colon3code.git",
      ]);
      ...
      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/t3tools/platform/colon3code");
      expect(identity?.displayName).toBe("t3tools/platform/colon3code");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/southbaghq/colon3code");
        expect(refreshedIdentity?.name).toBe("colon3code");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/southbaghq/colon3code");

      yield* git(cwd, [
        "remote",
        "set-url",
        "origin",
        "git@github.com:SouthbagHQ/colon3code-next.git",
      ]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/southbaghq/colon3code");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/southbaghq/colon3code-next");
      expect(refreshedIdentity?.displayName).toBe("southbaghq/colon3code-next");
      expect(refreshedIdentity?.name).toBe("colon3code-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
