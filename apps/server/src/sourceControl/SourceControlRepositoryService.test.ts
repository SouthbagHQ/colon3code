...
const CLONE_URLS = {
  nameWithOwner: "octocat/colon3code",
  url: "https://github.com/octocat/colon3code",
  sshUrl: "git@github.com:octocat/colon3code.git",
};
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    const result = yield* service.lookupRepository({
      provider: "github",
      repository: "octocat/colon3code",
      cwd: "/workspace",
    });

    assert.deepStrictEqual(result, { provider: "github", ...CLONE_URLS });
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/colon3code" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const destinationPath = path.join(parent, "colon3code");
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", "--progress", CLONE_URLS.url, "colon3code"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("reports clone progress from git's stderr and keeps its error text on failure", () =>
  Effect.gen(function* () {
    ...
    const stderrLines = [
      "Cloning into 'colon3code'...",
      "remote: Enumerating objects: 10, done.",
      "Receiving objects:  40% (4/10), 1.00 MiB | 2.00 MiB/s",
      "Receiving objects: 100% (10/10), 2.50 MiB | 2.00 MiB/s, done.",
      "fatal: early EOF",
      "fatal: unable to access 'https://user:s3c@ret@github.com/octocat/colon3code.git/': could not resolve host",
    ];
    ...
    assert.strictEqual(
      error.detail,
      "fatal: early EOF fatal: unable to access 'https://github.com/octocat/colon3code.git/': could not resolve host",
    );
  }).pipe(Effect.provide(NodeServices.layer)),
);
  ...
  Effect.gen(function* () {
    ...
    const result = yield* Effect.gen(function* () {
      ...
      return yield* service.prepareClone({
        remoteUrl: "https://user:s3cret@github.com/octocat/colon3code.git",
        destinationPath,
      });
    }).pipe(
      Effect.provide(
    ...
    assert.equal(result.remoteUrl, "https://github.com/octocat/colon3code.git");
    // Git itself still receives the credentials.
    assert.equal(result.cloneUrl, "https://user:s3cret@github.com/octocat/colon3code.git");
  }).pipe(Effect.provide(NodeServices.layer)),
);
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      const query = yield* service.prepareClone({
        remoteUrl: "https://github.com/octocat/colon3code.git?access_token=s3cret",
        destinationPath: path.join(parent, "a"),
      });
      assert.equal(query.remoteUrl, "https://github.com/octocat/colon3code.git");
      const nested = yield* service.prepareClone({
        remoteUrl: "https://user:pa@rt@github.com/octocat/colon3code.git",
        destinationPath: path.join(parent, "b"),
      });
      assert.equal(nested.remoteUrl, "https://github.com/octocat/colon3code.git");
    }).pipe(Effect.provide(makeLayer({})));
  }).pipe(Effect.provide(NodeServices.layer)),
);
...
it.effect("preserves destination probe failures instead of treating them as missing paths", () => {
  const fileSystemCause = PlatformError.systemError({
    ...
    module: "FileSystem",
    method: "exists",
    pathOrDescriptor: "/restricted/colon3code",
  });
  ...
  return Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.cloneRepository({
        ...
        destinationPath: "/restricted/colon3code",
      }),
    );
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/colon3code", visibility: "private" },
    ]);
