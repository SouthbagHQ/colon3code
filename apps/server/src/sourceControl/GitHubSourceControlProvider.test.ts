    ...
    const provider = yield* makeProvider({
      getPullRequest: () =>
        Effect.succeed({
          ...
          headRepositoryNameWithOwner: "fork/colon3code",
...
it.effect("adds safe request context while retaining GitHub CLI causes", () =>
  Effect.gen(function* () {
    ...
    const error = yield* provider
      .getChangeRequest({
        cwd: "/repo",
        reference: "https://user:secret@github.com/SouthbagHQ/colon3code/pull/42?token=secret#diff",
      })
      .pipe(Effect.flip);
    ...
    assert.deepStrictEqual(
      ...
      },
      {
        ...
        command: "gh",
        cwd: "/repo",
        reference: "https://github.com/SouthbagHQ/colon3code/pull/42",
