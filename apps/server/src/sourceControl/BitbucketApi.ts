...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("COLON3CODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("COLON3CODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("COLON3CODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("COLON3CODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
function checkoutBranchName(input: {
  ...
  return `colon3code/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
