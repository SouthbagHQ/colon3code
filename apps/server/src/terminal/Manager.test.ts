...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "colon3code-terminal-" });
...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            COLON3CODE_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              COLON3CODE_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.COLON3CODE_PORT).toBeUndefined();
  ...
  it.effect("strips AppImage runtime env from terminal sessions", () =>
    Effect.gen(function* () {
      const appDir = "/tmp/.mount_Colon3Codeabc123";
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: {
          APPIMAGE: "/home/user/Colon3-Code.AppImage",
          APPDIR: appDir,
          ARGV0: "/home/user/Colon3-Code.AppImage",
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            COLON3CODE_PROJECT_ROOT: "/repo",
            COLON3CODE_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
            NO_COLOR: "1",
            FORCE_COLOR: "0",
          },
      ...
      assert.equal(spawnInput.env.COLON3CODE_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.COLON3CODE_WORKTREE_PATH, "/repo/worktree-a");
  ...
  it.effect("resolves a provider instance environment before spawning", () =>
    Effect.gen(function* () {
      ...
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: { COLON3CODE_SECRET: "server-only" },
      ...
      expect(ptyAdapter.spawnInputs[0]?.env.COLON3CODE_SECRET).toBeUndefined();
        ...
        ServerSettings.layer.pipe(
          ...
          Layer.provide(
            ServerConfig.layerTest(process.cwd(), {
              prefix: "colon3code-terminal-provider-restart-",
            }),
          ),
        ),
      ),
