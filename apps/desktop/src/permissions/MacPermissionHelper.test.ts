...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/꞉3 Code (Nightly).app/Contents/MacOS/꞉3 Code",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/꞉3 Code.app/Contents/MacOS/꞉3 Code")).toBe(
      "/Applications/꞉3 Code.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/꞉3 Code.app/other/MacOS/꞉3 Code")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/꞉3 Code (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/꞉3 Code (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged :3 Code icon is missing");
  expect(windows).toHaveLength(0);
});
