    ...
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        const marker = yield* fileSystem.readFileString(
          path.join(root, "colon3code-wsl-server-tree.json"),
        );
  ...
  it.effect("sweeps stale version directories and leftover partials after extraction", () =>
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        // COLON3CODE_HOME is set to tempDir, so the desktop state dir resolves to
        // <tempDir>/userdata (no .colon3code segment).
