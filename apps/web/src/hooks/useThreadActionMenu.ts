...
export function useThreadActionMenu(input: {
  ...
  const { copyToClipboard: copyPathToClipboard } = useCopyToClipboard<{ path: string }>({
    onCopy: ({ path }) => {
      toastManager.add({ type: "success", title: "path copied :3", description: path });
    },
    onError: (error) => failureToast("failed to copy path 3:", error),
  });
  const { copyToClipboard: copyBranchToClipboard } = useCopyToClipboard<{ branch: string }>({
    target: "branch name",
    onCopy: ({ branch }) => {
      toastManager.add({ type: "success", title: "branch copied, purr :3", description: branch });
    },
    onError: (error) => failureToast("failed to copy branch 3:", error),
  });
  const { copyToClipboard: copyThreadIdToClipboard } = useCopyToClipboard<{ threadId: ThreadId }>({
    onCopy: ({ threadId }) => {
      toastManager.add({ type: "success", title: "thread ID copied :3", description: threadId });
    },
    onError: (error) => failureToast("failed to copy thread ID 3:", error),
  });
      ...
      void (async () => {
        ...
        if (action.startsWith("snooze:")) {
          ...
          if (result._tag === "Failure") {
            if (!isAtomCommandInterrupted(result)) {
              failureToast("failed to snooze thread 3: mrrp", squashAtomCommandFailure(result));
            }
            return;
          }
          toastManager.add(
            stackedThreadToast({
              type: "success",
              title: `snoozed until ${snoozeWakeDescription(preset.snoozedUntil, new Date(), timestampFormat)}, purr :3`,
              timeout: 5_000,
              actionProps: {
                children: "undo",
                onClick: () => {
                  void unsnoozeThread(threadRef).then((undone) => {
                    if (undone._tag === "Failure" && !isAtomCommandInterrupted(undone)) {
                      failureToast("failed to wake thread 3:", squashAtomCommandFailure(undone));
                    }
                  });
                },
              },
        ...
        switch (action) {
          ...
          case "new-thread-on-branch": {
            ...
            if (result._tag === "Failure") {
              failureToast("could not create thread 3: mrow", squashAtomCommandFailure(result));
            }
            return;
          }
          case "settle":
            await reportFailure("failed to settle thread 3:", () => settleThread(threadRef));
            return;
          case "unsettle":
            await reportFailure("failed to un-settle thread 3:", () => unsettleThread(threadRef));
            return;
          case "unsnooze":
            await reportFailure("failed to wake thread 3:", () => unsnoozeThread(threadRef));
            return;
          case "pin":
            await reportFailure("failed to pin thread 3: mrow", () => pinThread(threadRef));
            return;
          case "unpin": {
            await reportFailure("failed to unpin thread 3:", () =>
          ...
          case "regenerate-title":
            ...
            await reportFailure("failed to regenerate thread title 3: mrrp", () =>
          ...
          case "copy-path": {
            ...
            if (!workspacePath) {
              toastManager.add(
                stackedThreadToast({
                  type: "error",
                  title: "path unavailable 3: mrrp",
                  description: "this thread does not have a workspace path to copy, mrow.",
                }),
              );
              return;
            }
            ...
            if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
              failureToast(
                didArchive
                  ...
                  : "failed to archive thread 3: mrow",
          ...
          case "delete": {
            ...
            if (
              ...
              // that itself, and "failed to delete thread 3:" would be a lie.
              readThreadShell(threadRef) !== null
            ) {
              failureToast("failed to delete thread 3: mrrp", squashAtomCommandFailure(deleted));
            }
            return;
          }
          default:
