...
describe("pullRequestCandidateUrlFromReferenceAutolink", () => {
  it("turns GitHub's shared issue route into a pull request candidate", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/southbaghq/colon3code/issues/8600#issuecomment-1",
      ),
    ).toBe("https://github.com/southbaghq/colon3code/pull/8600#issuecomment-1");
  });

  it("does not reinterpret other issue hosts or malformed references", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://gitlab.com/southbaghq/colon3code/-/issues/8600",
      ),
    ).toBeNull();
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/southbaghq/colon3code/issues/not-a-number",
      ),
    ).toBeNull();
  });
});
...
describe("matchesLinkedPullRequestUrl", () => {
  ...
  it("matches the same pull request without looking up its project", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/SouthbagHQ/Colon3Code/pull/42/files",
      ),
    ).toBe(true);
  });
  ...
  it("keeps other providers' existing port normalization", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com:8443/southbaghq/colon3code/pull/42",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a different pull request or host", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/southbaghq/colon3code/pull/43",
      ),
    ).toBe(false);
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.example.com/southbaghq/colon3code/pull/42",
      ),
    ).toBe(false);
  });
});
...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/SouthbagHQ/Colon3Code/pull/123")).toEqual({
      host: "github.com",
      repository: "southbaghq/colon3code",
      number: 123,
    });
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/t3tools/platform/colon3code/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "t3tools/platform/colon3code",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/colon3code/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/colon3code",
      number: 17,
    });
    expect(
      parseChangeRequestUrl(
        "https://acme.visualstudio.com/platform/_git/colon3code/pullrequest/17",
      ),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/colon3code",
      number: 17,
    });
  });
  ...
  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(
      parseChangeRequestUrl("https://github.com/southbaghq/colon3code/pull/123/files?w=1"),
    ).toEqual({
      host: "github.com",
      repository: "southbaghq/colon3code",
      number: 123,
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/southbaghq/colon3code/issues/123",
      "https://github.com/southbaghq/colon3code/commit/0a1b2c3",
      "https://github.com/southbaghq/colon3code",
      "https://github.com/southbaghq/colon3code/pull/abc",
      "https://gitlab.com/southbaghq/colon3code/-/snippets/12",
      "https://gitlab.com/southbaghq/colon3code/-/issues/12",
      ...
      "javascript:alert(1)//github.com/southbaghq/colon3code/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `southbaghq/colon3code` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/t3tools/platform/colon3code",
        provider: "gitlab",
        displayName: "t3tools/platform/colon3code",
        owner: "t3tools",
        name: "colon3code",
      }),
    ];
