  ...
  const changeLink = async (threadId: ThreadId, remove: boolean) => {
    ...
    try {
      ...
      toastManager.add({
        type: "error",
        title: remove
          ? "oops, couldn't unlink the pull request 3:"
          : "oops, couldn't link the pull request 3:",
...
function ThreadPicker({
  ...
  return (
    <Command mode="none" value={query} onValueChange={setQuery} aria-label="choose a thread">
      <CommandInput placeholder="search threads or projects :3" disabled={pending} />
      <CommandList className="max-h-80 overflow-y-auto">
        {candidates.length === 0 ? (
          <div className="px-3 py-6 text-center text-sm text-muted-foreground">
            {" "}
            hmm, no active threads found ^w^
          </div>
        ) : (
