...
import { ChevronDownIcon, GitBranchIcon, SearchIcon } from "~/icons";
import {
  useCallback,
  useDeferredValue,
  useEffect,
  ...
  const copyBranchName = useCallback((branchName: string) => {
    void writeTextToClipboard(branchName, "branch name").then(
      (didCopy) => {
        ...
        toastManager.add({
          type: "success",
          title: "copied the branch name for you :3",
      ...
      (error: unknown) => {
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: "aw, couldn't copy the branch name 3:",
    ...
    runBranchAction(async () => {
      ...
      if (!isAtomCommandInterrupted(checkoutResult)) {
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: "aw, couldn't switch refs 3:",
    ...
    runBranchAction(async () => {
      ...
      if (!isAtomCommandInterrupted(createBranchResult)) {
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: "aw, couldn't create and switch to that ref 3:",
