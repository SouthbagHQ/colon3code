...
describe("work entry labels", () => {
  ...
  it.each([
    ...
    const browserEntry = {
      ...entry,
      toolTitle: "Colon3-code.preview_click",
      detail: '{"ok":true}',
      toolLifecycleStatus,
    };
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("folds the first assistant message and settled work before the terminal response", () => {
    ...
    expect(foldRow?.label).toBe("all done in 22s ^w^");
  ...
  it("derives a sane duration for a steer-superseded turn with one instant commentary message", () => {
    ...
    expect(foldRow?.turnId).toBe("turn-1");
    expect(foldRow?.label).toBe("all done in 12s ^w^");
  });
  ...
  it("uses latest-turn timings and the stopped label for an interrupted latest turn", () => {
    ...
    expect(rows).toEqual([
      expect.objectContaining({
        kind: "turn-fold",
        turnId: "turn-1",
        label: "you stopped me after 47s 3:",
        expanded: false,
      }),
    ]);
  });
    ...
    (isWorking) => {
      ...
      const timelineEntries = Array.from({ length: 1_000 }, (_, index) => ({
        ...
        entry: {
          ...
          createdAt,
          turnId,
          label: "colon3-code.preview_snapshot",
          tone: "tool" as const,
          toolLifecycleStatus:
