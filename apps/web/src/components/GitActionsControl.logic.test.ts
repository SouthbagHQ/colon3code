...
describe("resolveDefaultBranchActionDialogCopy", () => {
  it("uses push-only copy when pushing without a commit", () => {
    ...
    assert.deepEqual(copy, {
      ...
      description:
        'this action will push local commits on "main". you can keep going on this ref, or create a feature ref and run the same action there ^w^',
  ...
  it("uses push-and-pr copy when creating a PR without a commit", () => {
    ...
    assert.deepEqual(copy, {
      ...
      description:
        'this action will push local commits and create a pull request on "main". you can keep going on this ref, or create a feature ref and run the same action there ^w^',
  ...
  it("keeps commit copy when the action includes a commit", () => {
    ...
    assert.deepEqual(copy, {
      ...
      description:
        'this action will commit, push, and create a pull request on "main". you can keep going on this ref, or create a feature ref and run the same action there ^w^',
...
describe("resolveLiveThreadBranchUpdate", () => {
  ...
  it("does not regress a semantic thread ref back to a temporary worktree ref", () => {
    const update = resolveLiveThreadBranchUpdate({
      threadBranch: "colon3code/github-query-rate-limit",
      gitStatus: status({ refName: "colon3code/bda76797" }),
    });
  ...
  it("allows a temporary worktree ref to reconcile to a semantic branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      threadBranch: "colon3code/a9628676",
