...
// A draft with no source theme starts as the standard :3 Code look — the
...
export function ThemeEditorPanel({
  ...
  const handleSubmit = () => {
    if (!name.trim()) {
      setError("give your theme a name first :3");
      return;
    }
    ...
    try {
      ...
      if (
        ...
        setError("theme saved, but it couldn't be made active. want to try again? 3:");
        return;
      }
      onOpenChange(false);
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : isEditing
            ? "mrrp, couldn't save the theme 3:"
            : "aw, couldn't create the theme 3:",
      );
    }
  };
  ...
  const renderNameField = () => (
    <label className="grid grid-cols-[minmax(0,1fr)_minmax(0,2fr)] items-center gap-3">
      ...
      <Input
        ...
        placeholder={isEditing ? "theme name :3" : "e.g. Aurora"}
        value={name}
      />
    </label>
  );
