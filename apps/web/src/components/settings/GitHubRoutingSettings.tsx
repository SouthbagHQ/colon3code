  ...
  return (
    ...
    >
      <p className="px-3 py-2.5 text-xs text-muted-foreground sm:px-4">
        {" "}
        machines you trust here can peek at PR data through each other's GitHub access. enable both
        machines. read and act may use broader permissions than the machine that owns them. this
        only applies to this device :3
      </p>
      {environments.map((environment) => (
        <EnvironmentRow
              ...
              void update({ environmentId: environment.environmentId, permission }).then(
                (result) => {
                  ...
                  if (result._tag === "Failure")
                    toastManager.add({
                      type: "error",
                      title: "aw, that GitHub routing permission didn't save 3:",
                    });
                },
              );
            }}
