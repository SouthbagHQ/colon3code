          ...
          toastManager.add({
            ...
            title:
              savedEnvironmentCount > 0
                ? "setting saved on only some environments 3:"
                : "aw, that setting didn't save 3:",
            description: `couldn't update ${failedEnvironments.map((environment) => environment.label).join(", ")}.${savedEnvironmentCount > 0 ? " the other selected environments saved the change." : ""} 3:`,
          });
        },
      );
    },
