...
const buttonVariants = cva(
  "[--control-icon-color:currentColor] [&_svg]:-mx-0.5 relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-[var(--control-radius)] border font-medium text-base outline-none transition-[box-shadow,scale] [&:active:not([aria-haspopup])]:scale-[0.97] before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--control-radius)-1px)] pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-64 sm:text-sm [&_svg:not([class*='text-'])]:text-[var(--control-icon-color)] [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    ...
    variants: {
      size: {
        compact:
          "h-8 gap-1.5 rounded-md px-[calc(--spacing(3)-1px)] text-xs before:rounded-[calc(var(--radius-md)-1px)] [&_svg:not([class*='size-'])]:size-3.5",
        default: "h-10 px-[calc(--spacing(4)-1px)] sm:h-9",
        icon: "size-10 sm:size-9",
        "icon-lg": "size-11 sm:size-10",
        "icon-micro":
          "size-6 rounded-sm p-0 before:rounded-[calc(var(--radius-sm)-1px)] [&_svg:not([class*='size-'])]:size-3.5",
        "icon-tiny": "size-5 p-0 [&_svg:not([class*='size-'])]:size-3.5",
        "icon-sm": "size-9 sm:size-8",
        "icon-xl":
          "size-12 sm:size-11 [&_svg:not([class*='size-'])]:size-5 sm:[&_svg:not([class*='size-'])]:size-4.5",
        "icon-xs":
          "size-8 sm:size-7 not-in-data-[slot=input-group]:[&_svg:not([class*='size-'])]:size-4 sm:not-in-data-[slot=input-group]:[&_svg:not([class*='size-'])]:size-3.5",
        lg: "h-11 px-[calc(--spacing(4.5)-1px)] sm:h-10",
        micro:
          "h-6 gap-1 rounded-sm px-[calc(--spacing(2)-1px)] text-[11px] before:rounded-[calc(var(--radius-sm)-1px)] sm:text-[11px] [&_svg:not([class*='size-'])]:size-3 sm:[&_svg:not([class*='size-'])]:size-3",
        sm: "h-9 gap-1.5 px-[calc(--spacing(3.5)-1px)] sm:h-8",
        "sm-multiline":
          "min-h-9 gap-1.5 px-[calc(--spacing(3.5)-1px)] py-[calc(--spacing(2)-1px)] whitespace-normal sm:min-h-8",
        xl: "h-12 px-[calc(--spacing(5)-1px)] text-lg sm:h-11 sm:text-base [&_svg:not([class*='size-'])]:size-5 sm:[&_svg:not([class*='size-'])]:size-4.5",
        xs: "h-8 gap-1.5 px-[calc(--spacing(3)-1px)] text-sm sm:h-7 sm:text-xs [&_svg:not([class*='size-'])]:size-4 sm:[&_svg:not([class*='size-'])]:size-3.5",
      },
      variant: {
        chip: "",
        default:
