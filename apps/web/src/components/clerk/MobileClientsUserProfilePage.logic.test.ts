...
describe("mobile client presentation", () => {
  it("describes the client platform and enabled notification events", () => {
    ...
    expect(mobileClientPlatformLabel(client)).toBe("iOS 18 · :3 Code 1.2.3");
  ...
  it("distinguishes disabled notifications from an empty event selection", () => {
    ...
    ).toBe("push notifications are off on this device :3");
    expect(
      mobileClientNotificationDetail(
        device({
          notifications: {
    ...
    ).toBe("push notifications are on, but no alert types are picked yet ^w^");
  });
