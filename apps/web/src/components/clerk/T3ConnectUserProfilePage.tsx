          ...
          <div className="pt-3">
            ...
            >
              ...
              <p className="mt-4 max-w-xl text-[0.8125rem] leading-[1.125rem] text-muted-foreground">
                ...
                space will become available. local connections on your devices stay just as they are
                :3
              </p>
...
export function T3ConnectUserProfilePage() {
  ...
  const handleDeregister = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      ...
      toastManager.add({
        type: "success",
        title: "server deregistered, all done :3",
    ...
    toastManager.add({
      type: "error",
      title: "aw, couldn't deregister that server 3:",
      description: message,
      data: traceId
        ? {
  ...
  return (
    <ClerkUserProfilePage
      title="T3 Connect"
      description="the environments registered to your account — connections on this device live in settings :3"
      action={
        <ClerkUserProfileRefreshButton
      ...
      <div>
        ...
        {isInitialLoad ? (
          <p className="border-t py-4 text-[0.8125rem] text-muted-foreground" role="status">
            loading your environments, just a sec… :3
          </p>
          ...
          <Empty className="min-h-64 gap-4 border-t px-6 py-10 md:p-10">
            ...
            <EmptyHeader>
              <EmptyTitle className="text-[1.0625rem] leading-6">
                no T3 Connect environments yet, it’s quiet in here :3
              </EmptyTitle>
