...
describe("SidebarUpdateReleaseNotes", () => {
  ...
  it("links each preview to its exact release and labels hidden changes", () => {
    ...
    expect(anchors.map(({ props }) => props.href)).toEqual([
      "https://github.com/SouthbagHQ/colon3code/releases/tag/v0.0.36-nightly.3",
      "https://github.com/SouthbagHQ/colon3code/releases/tag/v0.0.36-nightly.2",
  ...
  it("links omitted releases to release history", () => {
    ...
    expect(anchors.at(-1)?.props.href).toBe("https://github.com/SouthbagHQ/colon3code/releases");
  ...
  it("reports a release link that fails to open", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(testState.addToast).toHaveBeenCalledWith({
        type: "error",
        title: "hmm, couldn't open the release notes 3:",
      });
    });
  });
});
