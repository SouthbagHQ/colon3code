  ...
  return (
    <main className="flex min-h-0 min-w-0 flex-1 items-center justify-center p-6">
      <div className="flex max-w-sm flex-col items-center gap-4 text-center">
        <h1 className="text-lg font-medium text-foreground">page not found 3: mrow</h1>
        <p className="text-sm text-muted-foreground">
          this link doesn't point to a page in {APP_DISPLAY_NAME}, mrrp. go home to choose a project
          or start a thread.
        </p>
...
function errorMessage(error: unknown): string {
  ...
  return "an unexpected router error occurred, mrow.";
}
...
function EventRouter({
  ...
  const handleServerConfigUpdated = useEffectEvent(() => {
    ...
    toastManager.add(
      stackedThreadToast({
        type: "warning",
        title: "invalid keybindings configuration 3: mrrp",
          ...
          onClick: () => {
            ...
            void (async () => {
              ...
              toastManager.add(
                stackedThreadToast({
                  type: "error",
                  title: "unable to open keybindings file 3: mrow",
                  description:
