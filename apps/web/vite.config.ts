...
const isSingleOriginDev = process.env.COLON3CODE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_COLON3CODE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.COLON3CODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: COLON3CODE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); COLON3CODE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.COLON3CODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // COLON3CODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.COLON3CODE_PORT, configuredWsUrl);
...
function devCompressionPlugin(): Plugin {
  return {
    name: "colon3code:dev-compression",
    apply: "serve",
...
const configuredAllowedHosts = (process.env.COLON3CODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_COLON3CODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
