...
function makeScope(projects: ReadonlyArray<EnvironmentProject>): HomeProjectScope {
  return {
    key: "github.com/SouthbagHQ/colon3code",
    title: ":3 Code",
...
describe("getProjectScopeSelectionTarget", () => {
  it("keeps the current environment when it hosts the selected logical project", () => {
    const projects = [
      makeProject("colon3code-mac", "mac"),
      makeProject("colon3code-server", "server"),
    ];
...
describe("resolveEnvironmentProjectMatch", () => {
  it("follows the same repository onto the target machine", () => {
    const selected = makeProject("colon3code", "mac", {
      repositoryKey: "github.com/SouthbagHQ/colon3code",
    });
    const target = [
      makeProject("other", "server", { repositoryKey: "github.com/t3tools/other" }),
      makeProject("colon3code-clone", "server", {
        repositoryKey: "github.com/SouthbagHQ/colon3code",
      }),
    ];
  ...
  it("falls back to workspace basename, then title, for unindexed projects", () => {
    const selected = makeProject("colon3code", "mac", { workspaceRoot: "/Users/me/colon3code" });
    const byBasename = [
      makeProject("other", "server"),
      makeProject("srv", "server", { workspaceRoot: "/home/me/colon3code" }),
    ];
    expect(resolveEnvironmentProjectMatch(byBasename, selected)).toBe(byBasename[1]);

    const byTitle = [
      makeProject("other", "server"),
      makeProject("srv", "server", { title: "colon3code" }),
    ];
  ...
  it("does not treat a known different repository as a basename or title match", () => {
    const selected = makeProject("colon3code", "mac", {
      repositoryKey: "github.com/SouthbagHQ/colon3code",
      workspaceRoot: "/Users/me/colon3code",
    });
    const fork = makeProject("fork", "server", {
      repositoryKey: "github.com/someone/colon3code",
      title: "colon3code",
      workspaceRoot: "/home/me/colon3code",
    });
    const unindexed = makeProject("unindexed", "server", { workspaceRoot: "/srv/colon3code" });
...
describe("resolveDraftProjectSelection", () => {
  it("preserves an explicit project selection", () => {
    const project = makeProject("colon3code");
    expect(
      resolveDraftProjectSelection("environment:colon3code", [project], [makeScope([project])]),
  ...
  it("selects one logical project even when it has multiple physical workspaces", () => {
    const projects = [
      makeProject("colon3code"),
      makeProject("colon3code-2"),
      makeProject("colon3code-3"),
    ];
