...
/**
 * Southbag Code (github.com/SouthbagHQ/code), a Pi-derived coding agent driven
 * over its JSONL RPC mode. Enabled by default: the provider probe only reports
 * it available when the `southbag-code` binary is installed, and when it is,
 * it is meant to be the preferred provider.
 */
export const SouthbagCodeSettings = makeProviderSettingsSchema(
  {
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("southbag-code").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Southbag Code binary.",
        providerSettingsForm: { placeholder: "southbag-code", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(CustomModelSetting).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
...
export type SouthbagCodeSettings = typeof SouthbagCodeSettings.Type;
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let :3 Code spawn the server when needed.",
        providerSettingsForm: {
...
/**
 * Cursor Cloud connection for this environment. The API key travels in
 * settings like provider environment secrets and the hub key above: it is
 * redacted before reaching a client and the real value lives in the secret
 * store. Mirroring is read-only, so one key per environment is enough.
 */
export const CursorCloudSettings = Schema.Struct({
  enabled: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
...
export const CursorCloudSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  apiKey: Schema.optionalKey(TrimmedString),
});
export type CursorCloudSettingsPatch = typeof CursorCloudSettingsPatch.Type;
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `colon3-code` server (and with it every
  ...
  providers: Schema.Struct({
    ...
    "southbag-code": SouthbagCodeSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const SouthbagCodeSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  customModels: Schema.optionalKey(Schema.Array(CustomModelSetting)),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      "southbag-code": Schema.optionalKey(SouthbagCodeSettingsPatch),
