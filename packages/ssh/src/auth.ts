...
const SSH_ASKPASS_DIR_NAME = "colon3code-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when :3 Code re-runs ssh with a cached password
...
printf ':3 Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when :3 Code re-runs\r
...
[Console]::Error.WriteLine(":3 Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "colon3code-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "colon3code" }),
  };
});
