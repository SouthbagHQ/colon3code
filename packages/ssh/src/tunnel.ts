...
if ! t3_runtime_ready; then
  ...
  # under it.
  T3_LOCK="$HOME/.colon3code/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_STAGING="$(mktemp -d "$HOME/.colon3code/runtime/versions/.staging-XXXXXX")"
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.colon3code/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.colon3code"
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env COLON3CODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
...
STATE_DIR="$HOME/.colon3code/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.colon3code"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
