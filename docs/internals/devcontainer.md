...
- The full dev stack: `vp run dev`, then open the pairing URL it prints through the forwarded web port (5733). The bare origin is useless without the pairing token. In VS Code the forwarded port is a true localhost, so the printed URL works as-is; in browser Codespaces the forwarded origin differs, and if the server rejects it, pass the forwarded origin via `COLON3CODE_DEV_ALLOWED_ORIGINS`.
...
`COLON3CODE_HOME` points at the workspace's gitignored `.colon3code`, so all runtime state stays inside the container workspace, mirroring the worktree default. There is no live install to damage inside a container, but the test-data rule from AGENTS.md still holds: copy data in, never point at shared state.
...
```bash
devcontainer build --workspace-folder . --push true --image-name <registry>/colon3code-devcontainer:latest
```
