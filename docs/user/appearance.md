...
On web and desktop, choose **Create theme** to adjust a palette, or import a :3 Code or VS Code
...
takes precedence. If the server stops publishing the selected theme, :3 Code falls back to its
standard theme.
...
Save a theme exported from :3 Code into `~/.colon3code/userdata/themes/` on the server, or the `themes`
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. :3 Code
