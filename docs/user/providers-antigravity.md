...
:3 Code runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for :3 Code to confirm
...
hostname. Only that :3 Code sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. :3 Code's separate Plan mode
...
:3 Code keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. :3 Code also reads `.gemini/skills` and the
