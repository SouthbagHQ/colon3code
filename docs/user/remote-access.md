...
Connect a phone, browser, or another desktop app to :3 Code running on a different
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. :3 Code starts
...
the connection stops a server that :3 Code launched; a server that was already
...
| Error                                                     | Recovery                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart :3 Code on the host.                                                                         |
...
| Expired or invalid link proof                             | Check the host's date and time, update :3 Code, then restart it.                                                                            |
...
`systemctl --user restart colon3code.service` for the background service. For a
...
devices. For server version warnings, follow [Updating :3 Code](./updating.md).
...
environment**. :3 Code restarts without a local server: no local agents or terminals run, WSL
