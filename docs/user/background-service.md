# Running :3 Code in the background

On Linux and macOS, :3 Code can run as a service for your user so you do not need
to keep a terminal open.

## Manage the service

Run these commands on the machine that will host :3 Code:
...
[Updating :3 Code](./updating.md).

Self-contained builds install as a download from the :3 Code GitHub release
...
`COLON3CODE_CHANNEL=nightly` for nightlies, `COLON3CODE_VERSION` to pin an exact
version, or `COLON3CODE_RELEASE_BASE_URL` to download from a mirror.
...
Linux needs systemd user services. Setup enables lingering so :3 Code starts at
...
with sudo; running :3 Code as root creates a separate installation and Connect
...
| `service-disabled` or `service-stopped` | Read the log and `systemctl --user status colon3code.service`, then use the repair command printed by :3 Code.                 |
...
`ProgramArguments` in
`~/Library/LaunchAgents/com.t3tools.colon3code.service.plist`.
