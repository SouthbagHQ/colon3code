...
:3 Code uses Claude Code's login and configuration. Start with the default provider
...
:3 Code uses the Claude configuration on the connected server.
...
Select the model you want in :3 Code. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in :3 Code. Custom models may have
