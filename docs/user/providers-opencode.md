...
:3 Code requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let :3 Code start OpenCode locally. A password in
provider settings applies to both that server and :3 Code's connection. With no
...
settings. :3 Code uses only that configured password for an external server; it
...
restart before :3 Code can see configuration changes.
