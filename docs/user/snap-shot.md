...
Shift keys together. :3 Code attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while :3 Code is in front captures :3 Code itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. :3 Code refuses shortcuts
...
terminal contents. If an app is slow to answer, :3 Code attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **:3 Code SnapShots** extension during setup. It is installed
...
shortcut does not fire, check :3 Code under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. :3 Code shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. :3 Code shows the exact binding it will
...
**Other Wayland desktops.** :3 Code uses the desktop's screenshot portal when it can capture the
