...
:3 Code has one server-side observability model:
...
```bash
export COLON3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export COLON3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export COLON3CODE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export COLON3CODE_TRACE_MIN_LEVEL=Info
export COLON3CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `COLON3CODE_OTLP_*`.

macOS app bundle example:

```bash
COLON3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
COLON3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
COLON3CODE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/꞉3 Code.app/Contents/MacOS/꞉3 Code"
```
...
```bash
TRACE_FILE="${COLON3CODE_HOME:-$HOME/.t3}/userdata/logs/server.trace.ndjson"
```
...
- `COLON3CODE_OTLP_TRACES_URL` was not set
...
- `COLON3CODE_TRACE_FILE`: override trace file path
- `COLON3CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `COLON3CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `COLON3CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `COLON3CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `COLON3CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `COLON3CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `COLON3CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `COLON3CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `COLON3CODE_OTLP_SERVICE_NAME`: service name, default `t3-server`
- `COLON3CODE_OTLP_HEADERS`: extra headers for both exporters, same format as
  `OTEL_EXPORTER_OTLP_HEADERS`: comma-separated `key=value` pairs with percent-encoded values.
- `COLON3CODE_OTLP_PROTOCOL`: `http/json` (default) or `http/protobuf`
