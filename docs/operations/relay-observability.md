...
- `colon3-code-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `colon3-code-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `colon3-code-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `colon3-code-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `colon3-code-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`colon3-code-relay-traces-dev-julius` for a personal stage.
