...
```dotenv
COLON3CODE_CLERK_PUBLISHABLE_KEY=<publishable key>
COLON3CODE_CLERK_JWT_TEMPLATE=<JWT template name>
COLON3CODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
COLON3CODE_RELAY_URL=https://relay.example.com
```
...
5. Set `COLON3CODE_CLERK_CLI_OAUTH_CLIENT_ID` to the generated public client ID in local and release
   build environments.
...
```json
{ "aud": "colon3-code-relay" }
```

Set `COLON3CODE_CLERK_JWT_TEMPLATE=t3-relay` for clients and
`CLERK_JWT_AUDIENCE=colon3-code-relay` for the relay. The production relay deployment environment
...
Development uses `colon3code-dev://app`; production uses `colon3code://app`. Update the array with
...
| Variant     | Callback                                          |
| ----------- | ------------------------------------------------- |
| Development | `clerk://com.t3tools.colon3code.dev.callback`     |
| Preview     | `clerk://com.t3tools.colon3code.preview.callback` |
| Production  | `clerk://com.t3tools.colon3code.callback`         |

Preserve existing entries. These callbacks are separate from the `colon3code-dev` / `colon3code-preview` / `colon3code` navigation schemes. A private development build using the production Clerk key still needs its development callback allowed by that instance's administrator; rebuilding the same package does not change the allowlist.

## Desktop passkeys

For a production macOS app with bundle ID `com.t3tools.colon3code`:
...
4. Check `https://<frontend-api>/.well-known/apple-app-site-association`. Its
   `webcredentials.apps` must include `<TEAM_ID>.com.t3tools.colon3code`.
...
COLON3CODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/colon3code.provisionprofile
# Override only when the RP domain differs from the Clerk Frontend API hostname.
COLON3CODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```
...
COLON3CODE_PORT=13773 \
  "/Applications/꞉3 Code (Alpha).app/Contents/MacOS/꞉3 Code (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/꞉3 Code (Alpha).app"
codesign -d --entitlements :- "/Applications/꞉3 Code (Alpha).app"
```
