...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("colon3-code-relay-traces", "prod")).toBe(
      "colon3-code-relay-traces-prod",
    );
    expect(relayResourceNameForStage("colon3-code-relay-traces", "dev_julius")).toBe(
      "colon3-code-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "colon3coderelay.com", hash)).toBe(
      "prod-abcdef0123456789.colon3coderelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "colon3coderelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
