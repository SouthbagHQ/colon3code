  ...
  it.effect("queues Live Activity jobs with the device's APNs routing", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* deliveries.sendForTarget({
        target: {
          ...target,
          bundle_id: "com.t3tools.colon3code.preview",
  ...
  it.effect("sends signed jobs to the device's APNs environment and bundle topic", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(requests[0]?.headers["apns-topic"]).toBe(
        "com.t3tools.colon3code.preview.push-type.liveactivity",
      );
    }).pipe(
      Effect.provide(
        makeLayer({
          attempts,
          currentTargets: [
            { ...target, bundle_id: "com.t3tools.colon3code.preview", aps_environment: "sandbox" },
          ],
    ...
    for (const changed of ["bundle", "environment", "legacy"] as const) {
      it.effect(`routes ${kind} using current registration with ${changed} job metadata`, () => {
        ...
        const payload = makeApnsDeliveryJobPayload({
          ...
          ...(changed === "legacy"
            ? {}
            : { bundleId: "com.t3tools.colon3code.dev", apsEnvironment: "sandbox" as const }),
        ...
        return Effect.gen(function* () {
          ...
          expect(requests[0]?.headers["apns-topic"]).toBe(
            `${changed === "bundle" ? "com.t3tools.colon3code.preview" : "com.t3tools.colon3code.dev"}${kind === "live_activity_update" ? ".push-type.liveactivity" : ""}`,
          );
        }).pipe(
          Effect.provide(
            makeLayer({
              ...
              currentTargets: [
                {
                  ...
                  bundle_id:
                    changed === "bundle"
                      ? "com.t3tools.colon3code.preview"
