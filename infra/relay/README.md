...
The relay is intentionally not in the hot path for normal :3 Code traffic. After a client connects,
...
- Linking :3 Code environments to a cloud account.
...
```sh
vp run --filter colon3code-relay deploy
```
...
```sh
vp run --filter colon3code-relay deploy -- --stage prod
vp run --filter colon3code-relay deploy -- --env-file .env.local
```
