...
if [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  pkgname='colon3code-bin'
elif [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+-nightly\.[0-9]{8}\.[0-9]+$ ]]; then
  pkgname='colon3code-nightly-bin'
else
...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Colon3-Code-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit colon3code-icon.png
git config user.name 'colon3code-ci'
git config user.email 'colon3code-ci@users.noreply.github.com'
git add -A
