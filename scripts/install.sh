#!/bin/sh
# Installs the :3 Code CLI from a GitHub Release archive. Needs only sh, tar,
...
base_url="${COLON3CODE_RELEASE_BASE_URL:-https://github.com/${repo}/releases/download}"
t3_home="${COLON3CODE_HOME:-$HOME/.t3}"
bin_dir="${COLON3CODE_INSTALL_BIN_DIR:-$HOME/.local/bin}"
...
channel="${COLON3CODE_CHANNEL:-stable}"
version="${COLON3CODE_VERSION:-}"
if [ -z "$version" ]; then
  ...
  case "$channel" in
    ...
    *) fail "COLON3CODE_CHANNEL must be stable, nightly, or preview" ;;
  esac
  tmp_index="$(mktemp)"
  ...
  rm -f "$tmp_index"
  [ -n "$version" ] || fail "could not find a ${channel} release; set COLON3CODE_VERSION"
fi
case "$version" in
  *-preview.*)
    printf '%s\n' \
      ...
      "  Set COLON3CODE_CHANNEL=stable (the default) for a supported build." >&2
    if [ "$channel" != "preview" ] && [ -z "${COLON3CODE_VERSION:-}" ]; then
