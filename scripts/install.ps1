# Installs the :3 Code CLI from a GitHub Release archive on Windows. Needs
...
#
# The archive is unpacked into $COLON3CODE_HOME\runtime\versions\<version>, the
...
$channel = if ($env:COLON3CODE_CHANNEL) { $env:COLON3CODE_CHANNEL } else { "stable" }
$version = $env:COLON3CODE_VERSION
if (-not $version) {
  ...
  $tagPattern = switch ($channel) {
    ...
    default { Fail "COLON3CODE_CHANNEL must be stable, nightly, or preview" }
  }
  ...
  if (-not $tag) { Fail "could not find a $channel release; set COLON3CODE_VERSION" }
  $version = $tag.Substring(1)
}
if ($version -match '-preview\.') {
  Write-Warning "t3 $version is a preview build. Preview builds are cut by maintainers from unreleased branches to exercise the release pipeline. They can be broken, receive no fixes, and are never offered as updates. Set COLON3CODE_CHANNEL=stable (the default) for a supported build."
  if ($channel -ne "preview" -and -not $env:COLON3CODE_VERSION) {
