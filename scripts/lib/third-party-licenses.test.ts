...
async function createFixture(): Promise<{
  ...
  const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "colon3code-licenses-"));
...
describe("third-party license generation", () => {
  it("keeps the GhosttyKit notice pinned to the vendored framework revision", async () => {
    ...
    expect(config).toContain(
      "https://github.com/Yash-Singh1/ghostty/tree/colon3code/custom-io-ordered-feed",
    );
  });
