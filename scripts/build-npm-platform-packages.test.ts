  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@colon3code/t3-darwin-arm64", "@colon3code/t3-linux-x64", "t3"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@colon3code/t3-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@colon3code/t3-linux-x64");
      ...
      const darwinManifest = yield* decodeManifest(
        yield* fs.readFileString(
          path.join(fixture.outputDir, "@colon3code/t3-darwin-arm64/package.json"),
        ),
      );
      ...
      assert.deepStrictEqual(launcherManifest.optionalDependencies, {
        "@colon3code/t3-darwin-arm64": VERSION,
        "@colon3code/t3-linux-x64": VERSION,
      });
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@colon3code", "t3", "t3.tgz"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@colon3code/t3-linux-x64.tgz")],
      ...
      assert.include(unsupported.stderr, "https://github.com/SouthbagHQ/colon3code/releases");
    }),
  );
});
